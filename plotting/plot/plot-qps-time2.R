...
lower_time_window_s <- 100
upper_time_window_s <- 100
...
for (commit in unique(data$experiment_commit_hash)) {
  ...
  plot <- plot +
    ...
    scale_alpha_manual(values = c(1., 1., 0., 0., 0., 0.)) +
    geom_vline(
      data = plot.data %>% filter(benchmark_log_patch_time_s < 6),
  ...
  ggplot.save(plot + plot.theme.paper() +
              ...
              , paste("QPS-Time-N2-OneLatency", commit, sep="-"), width=7.5, height = 4, use.grid=FALSE)
}
